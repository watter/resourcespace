...
/**
* Generates a CSRF token based on the current session
* 
* The CSRF Token is generated by a cryptographically secure random number generator
* 
* @uses escape_check()
* @uses generateSecureKey()
* @uses sql_value()
* 
* @param string $session The current session hash created by ResourceSpace (see: generate_session_hash() )
* 
...
function generateCSRFToken($session)
    ...
    $session_escaped = escape_check($session);

    // Before we continue, make sure we don't have a token which is already being used by other users
    while(true)
        {
        $token = generateSecureKey(256);

        if(sql_value("SELECT count(ref) AS `value` FROM user WHERE csrf_token = '{$token}'", 0) == 0)
            {
            break;
            }
        }

    // Allowing one user at a time? Return the token
    if($randomised_session_hash)
        {
        return $token;
        }

    if(sql_value("SELECT count(ref) AS `value` FROM user WHERE `session` = '{$session_escaped}'", 0) > 1)
        {
        trigger_error("Multiple identical sessions found!");
        }

    // Don't onverwrite an existing CSRF token if session didn't expire!
    // Note: when a session expires, user should be asked to log in again, at which point it is checked against the 
    // session value we have in our user record. If different, it means it expired.
    $current_csrf_token = sql_value("SELECT csrf_token AS `value` FROM user WHERE `session` = '{$session_escaped}'", '');
...
* 
* @param string  $token  CSRF Token which has been previously generated by generateCSRFToken()
* 
* @return void
*/
function setCSRFTokenCookie($token)
    ...
    if(trim($token) === '')
        {
        trigger_error("CSRF token must have a value!");
        }

    rs_setcookie("CSRFToken", $token, 0, $baseurl_short, "", (substr($baseurl, 0, 5) == "https"), true);
...
/**
* Deletes the cookie for CSRF Token
* 
* @return void
*/
function unsetCSRFTokenCookie()
    {
    global $baseurl_short;

    rs_setcookie("CSRFToken", "", -1, $baseurl_short);
