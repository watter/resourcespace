    ...
    foreach($resources as $resource)
        ...
        if($template_tags == "")
         ...
         if(trim($falcon_link_default_tag) != "")
            {
            $upload_tags .= "," . str_replace("[ID]",$ref,$falcon_link_default_tag);
            }
        ...
        //exit(get_resource_path($ref,false,'',false,$resourcedata['file_extension']) . "&k=" . $key);
        ...
        debug("falcon_link: falcon_link_publish. Resource:" . $ref . " - Successfuly published. Falcon id# " . $falconid);
